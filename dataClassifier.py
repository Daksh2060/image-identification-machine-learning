...
def enhancedFeatureExtractorDigit(datum):
    ...
    How much of the screen the digit takes up
    How many pixels are on the top and rights (to count for non-symmetrical numbers)
    How many separate connected regions there are (such as 8 or 9)
    ##
    """
    features = basicFeatureExtractorDigit(datum)
    grid = datum.getPixels()
    blanks, filled, top_segment, right_segment = 0, 0, 0, 0
    left_start, top_start = None, None
    ...
    for i in range(len(grid)):
        ...
        for j in range(1, len(rows)):
            if rows[j] != 0:
                filled += 1
                if left_start is None or j < left_start:
                    left_start = j
                if j <= (len(grid) + 1) / 2:
                    top_segment += 1
            if rows[j] != rows[j - 1]:
    ...
    for k in range(len(grid[0])):
        columns = []
        for point in grid:
            columns.append(point[k])
        for l in range(1, len(columns)):
            if columns[k] != 0:
                filled += 1
                if top_start is None or l < top_start:
                    top_start = l
                if l <= (len(grid[0]) + 1) / 2:
                    right_segment += 1
    ...
    height = len(grid) - (top_start * 2)
    width = len(grid[0]) - (left_start * 2)

    features.update({-n: 1.0 if 285 > filled else 0.0 for n in range(5)})
    features.update({(n + 1) * 10: 1.0 if 0.62 > float(width) / height else 0.0 for n in range(10)})
    features.update({n: 1.0 if 0.26 > float(right_segment) / filled else 0.0 for n in range(10, 15)})
    features.update({n: 1.0 if blanks > 180 else 0.0 for n in range(5)})
